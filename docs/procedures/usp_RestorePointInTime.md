...
This procedure consumes the restore chain generated by [`[cfg].[usp_GetLatestBackupFiles]`](../../docs/procedures/usp_GetLatestBackupFiles.md), transforms it into an executable restore plan, applies each restore step in sequence, persists execution telemetry, and optionally validates database integrity through `DBCC CHECKDB`.
...
| @RunID | BIGINT OUTPUT | Returns the restore execution identifier stored in [`[log].[RestoreTestRun]`](../../sql/01_Tables/log.RestoreTestRun.md). |
...
4. Retrieve the restore chain from [`[cfg].[usp_GetLatestBackupFiles]`](../../docs/procedures/usp_GetLatestBackupFiles.md) 
5. Build executable restore commands for each step  
6. Prepare the target environment  
7. Persist restore header telemetry into [`[log].[RestoreTestRun]`](../../sql/01_Tables/log.RestoreTestRun.md)  
8. Execute each restore step while capturing execution detail  
9. Persist step-level telemetry into [`[log].[RestoreStepExecution]`](../../sql/01_Tables/log.RestoreStepExecution.md)
